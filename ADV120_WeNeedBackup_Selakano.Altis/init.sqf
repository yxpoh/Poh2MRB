...
player createDiaryRecord ["Diary", ["Quick Brief", "1. Blufor are not to load up on their vehicles until Safe Start.<br />2. IND must stay in the town until Safe Start. <br />3. Opfor can only load up on their vehicles before SafeStart.<br />4. This is supposed to kinda create the 'IND in panic', 'Blufor rushing to vehicles' and 'Opfor ready to roll' situations.<br />5. IND, Prepared for loud sound after loading into the map.<br />6. Just in case more than half of the IND end up dying, Blufor have a chance at getting a draw by wiping the Opfor, but only before time limit(if not lengthened)."]];

switch (side player) do {
	case WEST:{
		player createDiaryRecord ["Diary", ["Extract Ambushed Convoy", "Double-Time down to Selakano and extract all friendly units. Godspeed! \n(At least half of the them must survive.)"]];
